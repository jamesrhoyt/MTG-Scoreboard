 ...
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="mtgstyle.css">
     ...
     <?php
        ...
        function createGame($conn, $sql, $result)
        {
            //Set the game's "id" value.
            $GLOBALS['sql'] = "SELECT COUNT(*) AS count FROM games";
            $GLOBALS['result'] = $GLOBALS['conn']->query($GLOBALS['sql']);
            $game_id = $GLOBALS['result']['count'];
            ...
            //Set the game's "team_size" value.
            ...
            $GLOBALS['sql'] = "INSERT INTO games (`id`, `name`, `active`. `team_size`, `game_type`, `start_time`, `turns`) VALUES ('$game_id', 'Remote Game', '1', '$team_size', '$game_type', '$start_time', '0')";
            $GLOBALS['result'] = $GLOBALS['conn']->query($GLOBALS['sql']);
            ...
            for ($i = 0; $i < $count; $i++)
            {
                ...
                $GLOBALS['sql'] = "SELECT COUNT(*) AS count FROM teams";
                $GLOBALS['result'] = $GLOBALS['conn']->query($GLOBALS['sql']);
                $id = $GLOBALS['result']['count'];
                $id = intval($id);
                ...
                /*if($game_type == 1)
                {
                    ...
                    for ($j = 0; $j < $count; $j++)
                    {
                        ...
                        for ($k = 0; $k < $count; $k++)
                        {
                            $GLOBALS['sql'] = "SELECT COUNT(*) AS count FROM commander_dmg";
                            $GLOBALS['result'] = $GLOBALS['conn']->query($GLOBALS['sql']);
                            $cmdrCount = $GLOBALS['result']['count'];
                            $GLOBALS['sql'] = "INSERT INTO commander_dmg (`id`, `game_id`, `from_id`, `to_id`, `life`) VALUES ('$cmdrCount', '$game_id', '$_SESSION[players][$j]', '$_SESSION[players][$k]', '0')";
