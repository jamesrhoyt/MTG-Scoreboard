...
gameSetupPanel::gameSetupPanel()
{
	...
	addPlayerTextbox = tgui::TextArea::create();
	gui->add(addPlayerTextbox);
	addPlayerTextbox->getRenderer()->setFont(*tguifont_dejavu);
	addPlayerTextbox->setReadOnly(false);
	addPlayerTextbox->setMaximumCharacters(14);
	addPlayerTextbox->setTextSize(20);
	...
	//Make the TextBox's caret color black.
	addPlayerTextbox->getRenderer()->setCaretColor(sf::Color::Black);
	//Position the TextBox just underneath the "header".
	addPlayerTextbox->setPosition(background.getPosition().x + 20, background.getGlobalBounds().height * .09f);

	//Set up the Button to add Players to the Database.
	addPlayerButton = tgui::Button::create();
	gui->add(addPlayerButton);
	addPlayerButton->getRenderer()->setFont(*tguifont_dejavu);
	addPlayerButton->setTextSize(18);
	addPlayerButton->getRenderer()->setTextColor(sf::Color::Black);
	addPlayerButton->getRenderer()->setBorders(tgui::Borders(3));
	addPlayerButton->setText("ADD PLAYER");
	//Set the Button's "hover" properties.
	addPlayerButton->getRenderer()->setBackgroundColorHover(sf::Color::White);
	addPlayerButton->getRenderer()->setTextColorHover(sf::Color::Black);
	addPlayerButton->getRenderer()->setBorderColorHover(addPlayerButton->getRenderer()->getBorderColor());
	//Set the Button's "down" properties.
	addPlayerButton->getRenderer()->setBackgroundColorDown(sf::Color::Black);
	addPlayerButton->getRenderer()->setTextColorDown(sf::Color::White);
	addPlayerButton->getRenderer()->setBorderColorDown(addPlayerButton->getRenderer()->getBorderColor());
	//Set the Button's "focused" properties.
	addPlayerButton->getRenderer()->setBorderColorFocused(sf::Color::Yellow);
	//Set addPlayerTextbox's size relative to addPlayerButton.
	addPlayerTextbox->setSize(addPlayerButton->getSize().x * 2.1f, addPlayerButton->getSize().y);
	...
	addPlayerButton->onClick(&gameSetupPanel::addPlayerToTable, this);

	//Set up the header text for the Player List.
	playerListHeader.setFont(*sffont_dejavu);
	playerListHeader.setStyle(sf::Text::Underlined);
	playerListHeader.setFillColor(sf::Color::White);
	playerListHeader.setCharacterSize(32);
	...
	teamSizeList->getRenderer()->setFont(*tguifont_dejavu);
	...
	//Set the colors for the TextBox's arrow when it is hovered over.
	teamSizeList->getRenderer()->setArrowBackgroundColorHover(teamSizeList->getRenderer()->getArrowBackgroundColor());
	teamSizeList->getRenderer()->setArrowColorHover(sf::Color::Black);
	...
	gameTypeList->getRenderer()->setFont(*tguifont_dejavu);
	...
	//Set the colors for the TextBox's arrow when it is hovered over.
	gameTypeList->getRenderer()->setArrowBackgroundColorHover(gameTypeList->getRenderer()->getArrowBackgroundColor());
	gameTypeList->getRenderer()->setArrowColorHover(sf::Color::Black);
	...
	createGameButton->getRenderer()->setFont(*tguifont_dejavu);
	createGameButton->setTextSize(18);
	createGameButton->getRenderer()->setTextColor(sf::Color::Black);
	createGameButton->setText("CREATE");
	//Set the Button's "hover" properties.
	createGameButton->getRenderer()->setBackgroundColorHover(sf::Color::White);
	createGameButton->getRenderer()->setTextColorHover(sf::Color::Black);
	createGameButton->getRenderer()->setBorderColorHover(createGameButton->getRenderer()->getBorderColor());
	//Set the Button's "down" properties.
	createGameButton->getRenderer()->setBackgroundColorDown(sf::Color::Black);
	createGameButton->getRenderer()->setTextColorDown(sf::Color::White);
	createGameButton->getRenderer()->setBorderColorDown(createGameButton->getRenderer()->getBorderColor());
	//Set the Button's "focused" properties.
	createGameButton->getRenderer()->setBorderColorFocused(sf::Color::Yellow);
...
void gameSetupPanel::slideOutCoroutine(sf::Sprite& tab)
{
	...
	while (background.getPosition().x > window->getSize().x - background.getGlobalBounds().width)
	{
		...
		addPlayerTextbox->setPosition(addPlayerTextbox->getPosition() + tgui::Vector2f(-distance, 0));
		addPlayerButton->setPosition(addPlayerButton->getPosition() + tgui::Vector2f(-distance, 0));
		playerListHeader.move(-distance, 0);
		gameTypeHeader.move(-distance, 0);
		gameSetupButtonAdd.move(-distance, 0);
		gameTypeList->setPosition(gameTypeList->getPosition() + tgui::Vector2f(-distance, 0));
		teamSizeHeader.move(-distance, 0);
		teamSizeList->setPosition(teamSizeList->getPosition() + tgui::Vector2f(-distance, 0));
		createGameButton->setPosition(createGameButton->getPosition() + tgui::Vector2f(-distance, 0));
		for (int i = 0; i < (int)playerNumbers.size(); i++)
		{
			playerNumbers[i].move(-distance, 0);
			gameSetupButtonsRemove[i].move(-distance, 0);
			playerEntryFields[i]->setPosition(playerEntryFields[i]->getPosition() + tgui::Vector2f(-distance, 0));
			if (teamSizeList->getSelectedItemIndex() > 0)
			{
				playerEntryFields2[i]->setPosition(playerEntryFields2[i]->getPosition() + tgui::Vector2f(-distance, 0));
			}
			if (teamSizeList->getSelectedItemIndex() == 2)
			{
				playerEntryFields3[i]->setPosition(playerEntryFields3[i]->getPosition() + tgui::Vector2f(-distance, 0));
			}
		}
	...
	if (background.getPosition().x <= window->getSize().x - background.getGlobalBounds().width)
	{
		...
		addPlayerTextbox->setPosition(addPlayerTextbox->getPosition() + tgui::Vector2f(difference, 0));
		addPlayerButton->setPosition(addPlayerButton->getPosition() + tgui::Vector2f(difference, 0));
		playerListHeader.move(difference, 0);
		gameTypeHeader.move(difference, 0);
		gameSetupButtonAdd.move(difference, 0);
		gameTypeList->setPosition(gameTypeList->getPosition() + tgui::Vector2f(difference, 0));
		teamSizeHeader.move(difference, 0);
		teamSizeList->setPosition(teamSizeList->getPosition() + tgui::Vector2f(difference, 0));
		createGameButton->setPosition(createGameButton->getPosition() + tgui::Vector2f(difference, 0));
		for (int i = 0; i < (int)playerNumbers.size(); i++)
		{
			playerNumbers[i].move(difference, 0);
			gameSetupButtonsRemove[i].move(difference, 0);
			playerEntryFields[i]->setPosition(playerEntryFields[i]->getPosition() + tgui::Vector2f(difference, 0));
			if (teamSizeList->getSelectedItemIndex() > 0)
			{
				playerEntryFields2[i]->setPosition(playerEntryFields2[i]->getPosition() + tgui::Vector2f(difference, 0));
			}
			if (teamSizeList->getSelectedItemIndex() == 2)
			{
				playerEntryFields3[i]->setPosition(playerEntryFields3[i]->getPosition() + tgui::Vector2f(difference, 0));
			}
		}
	}
	return;
...
void gameSetupPanel::addPlayerToTable()
{
	...
	sql::SQLString newPlayerName = sf::String(addPlayerTextbox->getText());
	...
	{
		playerEntryFields[i]->addItem(tgui::String(newPlayerName), std::to_string(newPlayerID));
		if (teamSizeList->getSelectedItemIndex() > 0)
		{
			playerEntryFields2[i]->addItem(tgui::String(newPlayerName), std::to_string(newPlayerID));
		}
		if (teamSizeList->getSelectedItemIndex() == 2)
		{
			playerEntryFields3[i]->addItem(tgui::String(newPlayerName), std::to_string(newPlayerID));
		}
	}
}
...
void gameSetupPanel::addPlayerEntryField(int index)
{
	...
	playerNumbers.push_back(sf::Text(std::to_string(index + 1) + ".", *sffont_dejavu, 32));
	...
	playerEntryFields[index]->getRenderer()->setFont(*tguifont_dejavu);
	...
	playerEntryFields[index]->setSize(playerNumbers[0].getGlobalBounds().width * 12, playerNumbers[index].getGlobalBounds().height * 1.2f);
	//Set the colors for the TextBox's arrow when it is hovered over.
	playerEntryFields[index]->getRenderer()->setArrowBackgroundColorHover(playerEntryFields[index]->getRenderer()->getArrowBackgroundColor());
	playerEntryFields[index]->getRenderer()->setArrowColorHover(sf::Color::Black);
	//Set the Text color for the TextBox.
	playerEntryFields[index]->getRenderer()->setTextColor(sf::Color::Black);
	...
	{
		res->next();
		playerEntryFields[index]->addItem(tgui::String(res->getString("name")), std::to_string(i));
	}
...
void gameSetupPanel::getUserInput(sf::Event event)
{
	...
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
	{
		...
		for (int i = 0; i < (int)playerEntryFields.size(); i++)
		{
			...
			if (playerEntryFields[i]->isMouseOnWidget(mousePos))
			{
			...
			{
				//If a Field was clicked on, make its Border yellow.
				if (playerEntryFields2[i]->isMouseOnWidget(mousePos))
				{
				...
				{
					//If a Field was clicked on, make its Border yellow.
					if (playerEntryFields3[i]->isMouseOnWidget(mousePos))
					{
...
std::vector<std::string> gameSetupPanel::getNames()
{
	...
	for (int i = 0; i < (int)playerEntryFields.size(); i++)
	{
		names.push_back(playerEntryFields[i]->getSelectedItem().toStdString());
		//If the team size is 2 or more, push the 2nd player on the team.
		if (teamSizeList->getSelectedItemIndex() > 0)
		{
			names.push_back(playerEntryFields2[i]->getSelectedItem().toStdString());
			//If the team size is 3, push the 3rd player on the team.
			if (teamSizeList->getSelectedItemIndex() == 2)
			{
				names.push_back(playerEntryFields3[i]->getSelectedItem().toStdString());
			}
		}
	}
	return names;
