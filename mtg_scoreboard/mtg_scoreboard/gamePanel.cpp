...
gamePanel::gamePanel(std::vector<std::string> names, std::vector<int> ids, int gameType, int teamSize)
{
	...
	for (int i = 0; i < (int)names.size(); i++)
	{
		...
		playerList[playerList.size() - 1].lifeCounter.setFont(*sffont_dejavu);
		...
		playerList[playerList.size() - 1].poisonCounter.setFont(*sffont_dejavu);
		...
		for (int i = 0; i < (int)names.size() / (teamSize + 1); i++)
		{
			...
			playerList[playerList.size() - 1].cmdrDmgCounters.push_back(sf::Text(std::to_string(playerList[playerList.size() - 1].cmdrDmg[i]), *sffont_dejavu));
		...
		playerList[playerList.size() - 1].displayName = tgui::TextArea::create();
		gui->add(playerList[playerList.size() - 1].displayName);
		playerList[playerList.size() - 1].displayName->getRenderer()->setFont(*tguifont_mtg);
		...
		if (teamSize > 0)
		{
			playerList[playerList.size() - 1].partnerDisplayNameOne = tgui::TextArea::create();
			gui->add(playerList[playerList.size() - 1].partnerDisplayNameOne);
			playerList[playerList.size() - 1].partnerDisplayNameOne->getRenderer()->setFont(*tguifont_mtg);
		...
		if (teamSize == 2)
		{
			playerList[playerList.size() - 1].partnerDisplayNameTwo = tgui::TextArea::create();
			gui->add(playerList[playerList.size() - 1].partnerDisplayNameTwo);
			playerList[playerList.size() - 1].partnerDisplayNameTwo->getRenderer()->setFont(*tguifont_mtg);
	...
	cmdrHoverText = tgui::TextArea::create();
...
gamePanel::gamePanel(int gameID, std::string gameName)
{
	...
	gameDisplayName.setFont(*sffont_mtg);
	...
	res = stmt->executeQuery(sql::SQLString("SELECT team_id, name, life, player_id, partner1_id, partner2_id, poison, isAlive FROM teams WHERE game_id=").append(std::to_string(gameID)).append(" ORDER BY team_id ASC"));
	for (int i = 0; i < numberOfPlayers; i++)
	{
		...
		//Set up the Life Counter (except size and position).
		playerList[i].lifeCounter.setFont(*sffont_dejavu);
		...
		//Set up the Poison Counter (except size and position).
		playerList[i].poisonCounter.setFont(*sffont_dejavu);
		...
		if (gameTypeID == 1)
		{
			...
			for (int j = 0; j < (int)commanderDamage->rowsCount(); j++)
			{
				...
				playerList[i].cmdrDmgCounters.push_back(sf::Text(std::to_string(playerList[i].cmdrDmg[j]), *sffont_dejavu));
		...
		playerName->next();
		playerList[i].displayName = tgui::TextArea::create();
		gui->add(playerList[i].displayName);
		playerList[i].displayName->getRenderer()->setFont(*tguifont_mtg);
		playerList[i].displayName->setReadOnly(true);
		playerList[i].displayName->getRenderer()->setBackgroundColor(sf::Color::Color(
		...
		playerList[i].displayName->getRenderer()->setTextColor(sf::Color::Color(
		...
		if (res->getInt("player_id") != res->getInt("partner1_id"))
		{
			...
			playerName->next();
			playerList[i].partnerDisplayNameOne = tgui::TextArea::create();
			gui->add(playerList[i].partnerDisplayNameOne);
			playerList[i].partnerDisplayNameOne->getRenderer()->setFont(*tguifont_mtg);
			playerList[i].partnerDisplayNameOne->setReadOnly(true);
			playerList[i].partnerDisplayNameOne->getRenderer()->setBackgroundColor(sf::Color::Color(
				sf::Uint8(strtol(playerName->getString("background_color").substr(0, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("background_color").substr(2, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("background_color").substr(4, 2).c_str(), NULL, 16))));
			playerList[i].partnerDisplayNameOne->getRenderer()->setBorderColor(sf::Color::Transparent);
			playerList[i].partnerDisplayNameOne->getRenderer()->setTextColor(sf::Color::Color(
				sf::Uint8(strtol(playerName->getString("text_color").substr(0, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("text_color").substr(2, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("text_color").substr(4, 2).c_str(), NULL, 16))));
			...
			playerList[i].teamDisplayName = tgui::TextArea::create();
			gui->add(playerList[i].teamDisplayName);
			playerList[i].teamDisplayName->getRenderer()->setFont(*tguifont_mtg);
		...
		if (res->getInt("player_id") != res->getInt("partner2_id") && res->getInt("partner1_id") != res->getInt("partner2_id"))
		{
			...
			playerName->next();
			playerList[i].partnerDisplayNameTwo = tgui::TextArea::create();
			gui->add(playerList[i].partnerDisplayNameTwo);
			playerList[i].partnerDisplayNameTwo->getRenderer()->setFont(*tguifont_mtg);
			playerList[i].partnerDisplayNameTwo->setReadOnly(true);
			playerList[i].partnerDisplayNameTwo->getRenderer()->setBackgroundColor(sf::Color::Color(
				sf::Uint8(strtol(playerName->getString("background_color").substr(0, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("background_color").substr(2, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("background_color").substr(4, 2).c_str(), NULL, 16))));
			playerList[i].partnerDisplayNameTwo->getRenderer()->setBorderColor(sf::Color::Transparent);
			playerList[i].partnerDisplayNameTwo->getRenderer()->setTextColor(sf::Color::Color(
				sf::Uint8(strtol(playerName->getString("text_color").substr(0, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("text_color").substr(2, 2).c_str(), NULL, 16)),
				sf::Uint8(strtol(playerName->getString("text_color").substr(4, 2).c_str(), NULL, 16))));
...
void gamePanel::setPosition(int x, int y)
{
	...
	if (remoteGameID != -1)
	{
		...
		for (int i = 0; i < (int)playerList.size(); i++)
		{
			...
			playerList[i].lifeCounter.setPosition(background.getPosition().x + (background.getGlobalBounds().width / 2) + xOffset - playerList[i].lifeCounter.getLocalBounds().left,
				background.getPosition().y + (background.getGlobalBounds().height / 2) + yOffset - playerList[i].lifeCounter.getLocalBounds().top);
			...
			if (gameTypeID == 1)
			{
				...
				if (!playerList[i].cmdrCountersExpanded)
				{
					...
					{
						playerList[i].cmdrDmgCounters[j].setPosition(playerList[i].lifeCounter.getPosition().x + (playerList[i].lifeCounter.getGlobalBounds().width / 2), playerList[i].poisonCounter.getPosition().y);
						...
						playerList[i].cmdrButtons[j].setPosition(playerList[i].cmdrDmgCounters[j].getPosition().x - playerList[i].cmdrDmgCounters[j].getLocalBounds().left, playerList[i].cmdrDmgCounters[j].getPosition().y + (playerList[i].cmdrDmgCounters[j].getGlobalBounds().height * .75f) + playerList[i].cmdrDmgCounters[j].getLocalBounds().top);
					}
				}
...
void gamePanel::movePlayer(int player, float xDiff, float yDiff)
{
	...
	playerList[player].displayName->setPosition(playerList[player].displayName->getPosition() + tgui::Vector2f(xDiff, yDiff));
	//Move the Avatar.
	...
	if (teamSizeID > 0)
	{
		...
		playerList[player].partnerDisplayNameOne->setPosition(playerList[player].partnerDisplayNameOne->getPosition() + tgui::Vector2f(xDiff, yDiff));
		...
		if (remoteGameID >= 0)
		{
			playerList[player].teamDisplayName->setPosition(playerList[player].teamDisplayName->getPosition() + tgui::Vector2f(xDiff, yDiff));
		}
		//Move the 2nd Partner's objects.
		if (teamSizeID == 2)
		{
			//Move the 2nd Partner's Display Name.
			playerList[player].partnerDisplayNameTwo->setPosition(playerList[player].partnerDisplayNameTwo->getPosition() + tgui::Vector2f(xDiff, yDiff));
		...
		for (int i = 0; i < (int)playerList.size(); i++)
		{
			...
			if (gameTypeID == 1)
			{
				for (int j = 0; j < (int)playerList[i].cmdrDmgCounters.size(); j++)
				{
					...
					playerList[i].cmdrDmgCounters[j].setOrigin((playerList[i].cmdrDmgCounters[j].getGlobalBounds().width / 2) + playerList[i].cmdrDmgCounters[j].getLocalBounds().left, (playerList[i].cmdrDmgCounters[j].getGlobalBounds().height / 2) + playerList[i].cmdrDmgCounters[j].getLocalBounds().top);
				...
				playerList[i].cmdrCloseButton.setScale(playerList[i].cmdrButtons[0].getScale() * .8f);
			}
			//Set the origin points for each of the visual elements (except the Display Names) to their centers.
			playerList[i].lifeCounter.setOrigin((playerList[i].lifeCounter.getGlobalBounds().width / 2) + playerList[i].lifeCounter.getLocalBounds().left, (playerList[i].lifeCounter.getGlobalBounds().height / 2) + playerList[i].lifeCounter.getLocalBounds().top);
			...
			playerList[i].poisonCounter.setOrigin((playerList[i].poisonCounter.getGlobalBounds().width / 2) + playerList[i].poisonCounter.getLocalBounds().left, (playerList[i].poisonCounter.getGlobalBounds().height / 2) + playerList[i].poisonCounter.getLocalBounds().top);
				...
				for (int j = 0; j < (int)playerList[i].cmdrDmgCounters.size(); j++)
				{
					...
					if (playerList[i].cmdrAvatars[j].getGlobalBounds().contains(mousePos) && playerList[i].cmdrDmgCounters[j].getFillColor().a > 0)
					{
						...
						if (cmdrHoverText->getText() != playerList[j].displayName->getText())
						{
							...
							cmdrHoverText->getRenderer()->setTextColor(playerList[j].displayName->getRenderer()->getTextColor());
							cmdrHoverText->getRenderer()->setBackgroundColor(playerList[j].displayName->getRenderer()->getBackgroundColor());
							cmdrHoverText->setText(playerList[j].displayName->getText());
							cmdrHoverText->setTextSize(playerList[j].displayName->getTextSize());
							cmdrHoverText->setSize(playerList[j].displayName->getSize());
						}
						//Make the hover text follow the mouse cursor.
						cmdrHoverText->setPosition(tgui::Layout2d(playerList[i].cmdrAvatars[j].getPosition()));
...
void gamePanel::AddPlayer()
{
	...
	for (int i = 0; i < (int)playerList.size(); i++)
	{
		...
		if (i != playerList.size() - 1)
		{
			...
			playerList[i].cmdrDmgCounters.push_back(sf::Text(std::to_string(playerList[i].cmdrDmg[playerList.size() - 1]), *sffont_dejavu));
...
void gamePanel::disableGUI()
{
	for (int i = 0; i < (int)playerList.size(); i++)
	{
		...
		if (teamSizeID > 0)
		{
			playerList[i].partnerDisplayNameOne->setEnabled(false);
		}
		if (teamSizeID == 2)
		{
			playerList[i].partnerDisplayNameTwo->setEnabled(false);
		}
	}
}
...
void gamePanel::enableGUI()
{
	for (int i = 0; i < (int)playerList.size(); i++)
	{
		...
		if (teamSizeID > 0)
		{
			playerList[i].partnerDisplayNameOne->setEnabled(true);
		}
		if (teamSizeID == 2)
		{
			playerList[i].partnerDisplayNameTwo->setEnabled(true);
		}
	}
}